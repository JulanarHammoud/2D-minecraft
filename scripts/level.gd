	...
	for x in range(width):
		for y in range(height):
			if y % 2 == 0:  # Even rows
				...
				if x > 0:  # Offset odd rows by 1 tile
...
func load_level():
	# If no save exists, just generate a new level
